...
logger = logging.getLogger("analyzer.parser.build_analyzer")

class BuildAnalyzer:
    """빌드 파일 분석 클래스"""
    
    def analyze(self, build_files):
        """빌드 파일 분석 수행"""
        # 기본 프로젝트 정보
        project_info = {
            'plugins': [],
        ...
        for file_info in build_files:
            if file_info['path'].endswith('.xml'):
                # Maven 빌드 파일
                maven_info = self.parse_maven_file(file_info['content'])
                self.merge_project_info(project_info, maven_info)
            else:
                # Gradle 빌드 파일
                gradle_info = self.parse_gradle_file(file_info['content'])
                self.merge_project_info(project_info, gradle_info)
        
        return project_info
    
    def merge_project_info(self, target, source):
        """두 프로젝트 정보 병합"""
    ...
    def parse_gradle_file(self, content):
    ...
    def parse_maven_file(self, content):
